...
typedef struct _nesnoise  // Data structure for this object 
{     
  t_object x_obj;
  float x_f; //PD
...
extern void apu_create(t_nesnoise *x, int cpunum, double basefreq, int sample_rate, int refresh_rate, int sample_bits);
extern void apu_setparams(t_nesnoise *x, int sample_rate, int refresh_rate, int sample_bits);

extern void apu_process(t_nesnoise *x, t_float *buffer, int num_samples);
extern void apu_reset(t_nesnoise *x);

extern void apu_setfilter(t_nesnoise *x, int filter_type);
extern void apu_setchan(int chan, boolean enabled);

extern uint8 apu_read(t_nesnoise *x, uint32 address);
extern void apu_write(t_nesnoise *x, uint32 address, uint8 value);
