/* 
 ...
 http://web.mit.edu/tbaran/www/autotuned.html
 VERSION 0.1
 May 31, 2009
 
 MaxMSP port by oli larkin
 PD port, fork and modifications by maxus germanus 2011
 ...
 - fixed crash when deleting the plugin with dsp on
...
void autotuned_tilde_setup(void)
{
	...
	post("autotuned~ v.0.1");
	post("maxus germanus 2011");
}
...
/*		if (x->phasein >= 1) {
	...
	x->frag[(ti+N)%N] = x->cbf[(ti + ti2 + N)%N];
      }
    }
*/	
...
*/
		...
		*(out++) = fMix*tf + (1-fMix)*x->cbi[(x->cbiwr - N + 1)%N];
		//*(pfOutput++) = (LADSPA_Data) fMix*tf + (1-fMix)*x->cbi[(x->cbiwr - N + 1)%N];
