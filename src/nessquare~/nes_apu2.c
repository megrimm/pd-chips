...
static int32 apu_rectangle(t_nes_square *x, rectangle_t *chan)
{
   int32 output;
...
static int32 apu_triangle(t_nes_square *x, triangle_t *chan)
{
   APU_VOLUME_DECAY(chan->output_vol);
...
static int32 apu_noise(t_nes_square *x, noise_t *chan)
{
...
static int32 apu_dmc(t_nes_square *x, dmc_t *chan)
{
   int delta_bit;
...
void apu_write(t_nes_square *x, uint32 address, uint8 value)
{
   int chan;
   apu_t *apu = &(x->apu);
...
/* Read from $4000-$4017 */
uint8 apu_read(t_nes_square *x, uint32 address)
{
   uint8 value;
...
void apu_getpcmdata(t_nes_square *x, int *num_samples, int *sample_bits)
{
   //ASSERT(apu);
   //*data = apu->buffer;
...
void apu_process(t_nes_square *x, t_float *buffer, int num_samples)
{
   //FIX FIX FIX FIX!!
   ...
   if (NULL != buffer)
   {
      while (num_samples--)
      {
         ...
         if (apu->mix_enable[1]) accum += apu_rectangle(x, &apu->rectangle[1]);
...
/* set the filter type */
void apu_setfilter(t_nes_square *x, int filter_type)
{
   (x->apu).filter_type = filter_type;
}

void apu_reset(t_nes_square *x)
{
   uint32 address;
...
void apu_build_luts(t_nes_square *x, int num_samples)
{
   int i;
...
void apu_setparams(t_nes_square *x, int sample_rate, int refresh_rate, int sample_bits)
{
   apu_t *apu = &(x->apu);
...
/* (1, 60, 44100, 60, 16)*/
void apu_create(t_nes_square *x, int cpunum, double basefreq, int sample_rate, int refresh_rate, int sample_bits)
{
   int channel;
